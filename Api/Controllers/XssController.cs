    ...
    public class XssController : ControllerBase
    {
        ...
        public async Task<IActionResult> XssScanner([FromBody] Website model, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // Set VulnerabilityId to a default XSS type (e.g., Reflected XSS) to ensure scan history identifies it correctly
                var defaultXssVuln = await _context.Vulnerabilities
                    .FirstOrDefaultAsync(v => v.VulnerabilityName == VulnerabilityTypes.ReflectedXSS, cancellationToken);
                if (defaultXssVuln != null)
                {
                    scanRequest.VulnerabilityId = defaultXssVuln.VulnerabilityId;
                }
                await _context.SaveChangesAsync(cancellationToken);
                ...
                await _context.SaveChangesAsync(cancellationToken);
                _logger.LogInformation($"Returning redirectUrl: /xss/scan-results/{scanRequest.RequestId}");

                return Ok(new
                {
                    Message = "Scan completed successfully!",
                    redirectUrl = $"/xss/scan-results/{scanRequest.RequestId}"
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("scan-results/{requestId}")]
        public async Task<IActionResult> GetXssScanResults([FromRoute] int requestId, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var alerts = await _xssZapService.GetProcessedScanResultsAsync(scanRequest.ZAPScanId ?? 0, cancellationToken);

                if (!alerts.Any())
                {
                    _logger.LogWarning($"No XSS vulnerabilities found for RequestId {requestId} on {scanRequest.Website.Url}");
            ...
            {
                _logger.LogWarning($"Request ID {requestId} not found in mapping.");
        ...
        public async Task<IActionResult> GetXssScanResultsByRequest([FromRoute] int requestId, CancellationToken cancellationToken)
        {
            try
            {
                var scanRequest = await _context.ScanRequests
                ...
                if (!scanResults.Any())
                {
                    _logger.LogWarning($"No XSS vulnerabilities found for RequestId {requestId} on {scanRequest.Website?.Url ?? "Unknown"}");
                    return Ok(new { Message = "No XSS vulnerabilities found.", Results = Array.Empty<ScanResult>() });
                }

                return Ok(new { Message = "XSS scan results retrieved successfully.", Results = scanResults });
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error retrieving scan results for RequestId {requestId}: {ex.Message}");
                return StatusCode(500, "Failed to retrieve scan results.");
            }
        }

        [HttpGet("scan-results/{requestId}/pdf")]
        public async Task<IActionResult> DownloadXssScanResultsPdf([FromRoute] int requestId, CancellationToken cancellationToken)
        {
            try
            {
                var scanRequest = await _context.ScanRequests
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    var websiteUrl = scanRequest.Website?.Url ?? "Unknown";
                    if (uniqueAlerts.Any() && !string.IsNullOrEmpty(uniqueAlerts.First().AffectedUrl))
                    {
                        ...
                        {
                            websiteUrl = uniqueAlerts.First().AffectedUrl;
                        }
                    }
                    ...
                    foreach (var group in groupedByType)
                    {
                        ...
                        foreach (var alert in group)
                        {
                            ...
                            detailsList.Add(new ListItem(new Phrase($"Payload: {alert.Payload}", sectionFont)));
                    ...
                    return File(pdfBytes, "application/pdf", $"XSS_Scan_Report_{requestId}.pdf");
                }
            }
            catch (Exception ex)
            {
