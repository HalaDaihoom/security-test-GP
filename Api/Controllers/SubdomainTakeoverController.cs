...
namespace Api.Controllers
{
    ...
    public class SubdomainTakeOverController : ControllerBase
    {
        ...
        private readonly string _toolsPath = "/app/tools"; // or wherever your Dockerfile installs the tools
        ...
        public async Task<IActionResult> CreateTakeoverScan([FromBody] DomainRequest request)
        {
            ...
            try
            {
                ...
                // Run assetfinder and subzy
                await RunCommandAndCaptureOutputAsync($"assetfinder --subs-only {domain} > {_toolsPath}/{subsFile}");

                string subzyOutput = await RunCommandAndCaptureOutputAsync($"subzy run --targets {_toolsPath}/{subsFile}");
