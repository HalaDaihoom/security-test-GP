...
// using Api.DTOs; // Add this at the top
// using Api.Services;
...
//             {
//                 scanId = await _zapService.StartScanAsync(model.Url, cancellationToken);
//                 scanRequest.ZAPScanId = int.Parse(scanId);; // Store the ScanId from ZAP
//                 await _context.SaveChangesAsync(cancellationToken);
//             }
//             catch (Exception ex)
//             {
...
//     //         // Deserialize the ZAP results into ZapAlert objects
//     //         var zapAlerts = JsonConvert.DeserializeObject<List<ZapAlert>>(scanResults);

//     //         if (zapAlerts == null || !zapAlerts.Any())
//     //             return Ok(new { Message = "No vulnerabilities found.", Results = new List<ScanResult>() });

//     //         // Map ZapAlert objects to ScanResult model
//     //         var scanResultsToSave = zapAlerts.Select(alert => new ScanResult
//     //         {
//     //             RequestId = int.Parse(scanId), // Assuming scanId maps to RequestId
//     //             VulnerabilityId = 0,          // You can map this if you have a Vulnerability table
//     //             Severity = alert.Risk,        // Map Risk to Severity
//     //             Details = JsonConvert.SerializeObject(alert) // Store full details as JSON
//     //         }).ToList();
...
//     {
//         // Fetch ZAP scan results
//         string scanResults = await _zapService.GetScanResultsAsync(scanId.ToString(), cancellationToken);
...
//         // Find the ScanRequest associated with the scanId
//         var scanRequest = await _context.ScanRequests
//             .FirstOrDefaultAsync(r => r.RequestId == scanId, cancellationToken);
